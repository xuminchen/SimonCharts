...
rc.49 adds strict Rich Study Inputs for bounded number, boolean, string, select, and Candle source values. The native Study panel creates and edits Custom Studies through the existing parser and action path; typed params continue through checkpoints, crosshair, Entity APIs, Layout V2/V3, failure/retry, and readiness without a second engine or dependency. Identity-free `ChartStudyPresetV1` snapshots only ordered Study configuration, while `applyStudyPreset()` validates atomically, replaces the current Studies, and returns fresh entity IDs. Preset naming and persistence remain host-owned.

rc.50 adds no public API or dependency. Ordinary K-line charts reserve 16% of the content area for 62%-opacity volume bars, while the existing overlay Canvas highlights only the active candle band and volume bar. Header OHLC values stay neutral and only price movement keeps directional color; the compact hover panel retains time, OHLC, change, and volume. One persistent timeframe indicator now slides across shortcut buttons and the More trigger with a 240 ms transform/width transition, recomputes after resize, and disables motion under `prefers-reduced-motion`.
...
The accepted immutable package artifact is `dist/packages/simoncharts-charts-1.0.0-rc.50.tgz` (8 files, 160,851 bytes), SHA-256 `145b2155097301282929a328bd3aa9db05a3d920406017f0a7696945a8fb7a6e`, SHA-512 `adfe06f6013f52951ecab3858c24815b32d00f40c0ed29ef4d0a75da555ffba26c3f2fc0a7646a803d79034ac4741870698f158fac78e0d212a7ec574c72cc5f`. Its package-only commercial gate passed `79 files / 1,372 tests`, combined Chrome `167/167`, Charts `23 files / 350 tests`, Charts Chrome/Edge `116/116` per browser, and exact packed JavaScript/TypeScript Chrome/Edge consumer execution. Historical artifacts remain immutable; authenticated reference-host acceptance remains the separate final commercial gate described below.
