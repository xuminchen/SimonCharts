...
export function createStepLineRenderer() {
  return createRenderer("stepLine", (renderContext) => {
    ...
    withPlotClip(renderContext, () => {
      ...
      for (let index = bounds.from; index <= bounds.to; index += 1) {
        ...
        if (index === bounds.from) {
          ...
          const previousY = yForPrice(renderContext, range, renderContext.model.points[index - 1].close);
